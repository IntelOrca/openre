...
//
// Input vertices are D3DTLVERTEX values in screen space (top-left origin,
// pixels, z in [0,1]). SDL_GPU uses the D3D12/Metal convention: NDC has its
// origin at the lower-left with +Y up, viewport coordinates are top-left
...
// top-left origin, exactly like the original D3D2 viewport mapping.
//
...
// assumes all D3D12 vertex inputs are TEXCOORDn and maps them to attribute
// locations in order).
...
VSOutput main(
    ...
    float4 aColor : TEXCOORD1,    // D3DCOLOR as UBYTE4_NORM: byte order B,G,R,A
    float2 aTexcoord : TEXCOORD2) // tu, tv (top-left origin)
{
    ...
    // aColor arrives in D3DCOLOR memory order (B,G,R,A); swizzle to RGBA.
    o.color = aColor.zyxw;
